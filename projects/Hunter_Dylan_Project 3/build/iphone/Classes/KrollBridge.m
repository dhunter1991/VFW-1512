...
NSString * Hunter_Dylan_Project_3$ModuleRequireFormat = @"(function(exports){"
...
@implementation Hunter_Dylan_Project_3Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_hunter_dylan_project_3);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Hunter_Dylan_Project_3 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Hunter_Dylan_Project_3" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_hunter_dylan_project_3 = [[Hunter_Dylan_Project_3Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_hunter_dylan_project_3];
	
	NSString *_hunter_dylan_project_3NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _hunter_dylan_project_3NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_hunter_dylan_project_3 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Hunter_Dylan_Project_3$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.hunter_dylan_project_3.kroll" 
