...
NSString * project1_dylan_hunter$ModuleRequireFormat = @"(function(exports){"
...
@implementation project1_dylan_hunterObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_project1_dylan_hunter);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create project1_dylan_hunter global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "project1_dylan_hunter" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_project1_dylan_hunter = [[project1_dylan_hunterObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_project1_dylan_hunter];
	
	NSString *_project1_dylan_hunterNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _project1_dylan_hunterNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_project1_dylan_hunter valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:project1_dylan_hunter$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.project1_dylan_hunter.kroll" 
