...
//Rare exceptions to indicate a bug in the _project1_dylan_hunter code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_project1_dylan_hunter = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_project1_dylan_hunter]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.project1_dylan_hunter.module"
